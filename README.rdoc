...
To use this you can put the following in your view erb file:

  <%= swfobject_tag "httppath/to/myfile.swf" =>

  <%= swfobject_tag "httppath/to/videplayer.swf" :width => 400, :height => 300, :variables => {:url => @video.url} %>
...
development mode.  In production, it will simply include the swf
allowing for the OpenLazlo server or compilation tools to be present
only in your dev enviroment.

  <%= applet_tag  "myplayer", :width => 400, :height => 300, :variables => {:url => @video.url} %>

In the above example the corresponding OpenLaszlo source file
(the .lzx file which declared the canvas) is
expected to be in /app/applets/myplayer/myplayer.lzx or /app/applets/myplayer/main.lzx
...
3: Install this plugin:
  $ ./script/plugin install git://github.com/ultrasaurus/openlaszlo_plugin.git
